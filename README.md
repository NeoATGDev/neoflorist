...
real backend would receive; nothing is actually ordered. Phase 2 (not in this
repo) is a Spring Boot API on an Oracle Cloud VM that Phase 1's `CartContext`
is already shaped to call.
...
src/
  ...
  context/CartContext.jsx ← cart state + the simulated "backend call"
  pages/
  ...
  styles.css
prerender.mjs             ← renders all 106 routes to static HTML + JSON-LD
vercel.json
```
